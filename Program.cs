...
namespace MultipleSign
{
    internal class Program
    {
                ...
                if (taskQueue.TryDequeue(out TaskData taskData))
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            consumerResult.Message = "失败，执行超时";
                            results.Add(consumerResult);
                        }
                    }
        ...
        private static async Task ProcessTaskAsync(TaskData task)
        {
            Console.WriteLine(Util.GetBeiJingTimeStr() + " - " + task.TaskId.ToString().PadLeft(4, '0') + " - Entry Process");

            if (implementations.TryGetValue(task.TaskItemEnum, out ISignConsumer implementation))
                await implementation.Consumer(task);
            else
