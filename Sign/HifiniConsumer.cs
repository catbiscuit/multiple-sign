...
namespace MultipleSign.Sign
{
    public class HifiniConsumer : ISignConsumer
    {
            ...
            if (conf.HifiniConf != null && conf.HifiniConf.Cookies != null)
            {
                ...
                foreach (var item in conf.HifiniConf.Cookies.Where(x => string.IsNullOrWhiteSpace(x) == false))
                {
                    tasks.Add(new TaskData()
                    {
                        ...
                        TaskItemEnum = TaskItemEnum.Hifini,
                        TaskItemSort = idx,
                        Parameter = item,
                    });
                    idx++;
        ...
        private async Task<string> SignV1(string cookie)
        {
        ...
        private async Task<string> SignV2(string cookie, string sign)
        {
        ...
        private async Task<string> SignV3(string cookie, string sign)
        {
            string url = "https://hifini.com/sg_sign.htm";
            var headers = new Dictionary<string, string>
            {
