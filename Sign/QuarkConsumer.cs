...
namespace MultipleSign.Sign
{
    public class QuarkConsumer : ISignConsumer
    {
            ...
            if (conf.QuarkConf != null && conf.QuarkConf.Accounts != null)
            {
                ...
                foreach (var item in conf.QuarkConf.Accounts.Where(x => string.IsNullOrWhiteSpace(x.User) == false || string.IsNullOrWhiteSpace(x.Kps) == false || string.IsNullOrWhiteSpace(x.Sign) == false || string.IsNullOrWhiteSpace(x.Vcode) == false))
                {
                    tasks.Add(new TaskData()
                    {
                        ...
                        TaskItemEnum = TaskItemEnum.Quark,
                        TaskItemSort = idx,
                        Parameter = item,
                    });
                    idx++;
        ...
        private async Task<string> DoSign(QuarkConfModel quarkConfModel)
        {
            ...
            var growth_info = await GetGrowthInfo(quarkConfModel);
            if (growth_info != null && growth_info["data"] != null)
            {
                ...
                {
                    var growth_sign = await GetGrowthSign(quarkConfModel);
                    ...
                    {
                        return "❌ 签到异常: " + (growth_sign?["message"]?.ToString() ?? "");
                    }
                }
            }
            else
        ...
        private async Task<JsonObject> GetGrowthInfo(QuarkConfModel quarkConfModel)
        {
        ...
        private async Task<JsonObject> GetGrowthSign(QuarkConfModel quarkConfModel)
        {
